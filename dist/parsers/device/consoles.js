    ...
    constructor() {
        this.parse = (userAgent) => {
            ...
            for (const [brand, gameConsole] of Object.entries(consoles_json_1.default)) {
                const match = (0, user_agent_1.userAgentParser)(gameConsole.regex, userAgent);
                if (!match)
                    continue;
                result.type = gameConsole.device;
                result.brand = brand;
                if ("model" in gameConsole && gameConsole.model) {
                    result.model = (0, variable_replacement_1.variableReplacement)(gameConsole.model, match).trim();
                }
