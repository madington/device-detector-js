    ...
    constructor() {
        this.parse = (userAgent) => {
            ...
            for (const [brand, mobile] of Object.entries(mobiles_json_1.default)) {
                const match = (0, user_agent_1.userAgentParser)(mobile.regex, userAgent);
                if (!match)
                    continue;
                resultType = "device" in mobile && mobile.device || "";
                result.brand = brand;
                if ("model" in mobile && mobile.model) {
                    result.model = (0, model_1.buildModel)((0, variable_replacement_1.variableReplacement)(mobile.model, match)).trim();
                }
