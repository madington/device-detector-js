    ...
    constructor() {
        this.parse = (userAgent) => {
            ...
            for (const [brand, portableMediaPlayer] of Object.entries(portable_media_player_json_1.default)) {
                const match = (0, user_agent_1.userAgentParser)(portableMediaPlayer.regex, userAgent);
                if (!match)
                    continue;
                result.type = portableMediaPlayer.device;
                result.brand = brand;
                if ("model" in portableMediaPlayer && portableMediaPlayer.model) {
                    result.model = (0, variable_replacement_1.variableReplacement)(portableMediaPlayer.model, match).trim();
                }
                    ...
                    for (const model of portableMediaPlayer.models) {
                        ...
                        if (!modelMatch)
                            continue;
                        result.model = (0, variable_replacement_1.variableReplacement)(model.model, modelMatch).trim();
                        break;
                    }
                }
                break;
